...
`amitbaz/job-hunter-bot` and `amitbaz/interviewer-app` are being left as-is for now.

Note that `job-hunter-bot` still has its own `daily.yml` on a 09:30 Europe/Berlin schedule. Once
the daily run works here, disable it there — otherwise both repositories run the hunt, send
Telegram messages, and write divergent state:
...
5. Trigger `job-hunter-daily.yml` manually and confirm it restores state, runs, and uploads.
6. Disable the old repository's schedules (§7).
