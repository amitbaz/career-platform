...
writes go through `PostgresJobStore` (`src/engine/postgres_store.py`), which reaches
...
- `src/engine/sources/` — public job discovery adapters: Remotive, Arbeitnow, Jobicy, Himalayas, Remote OK, We Work Remotely, Hacker News, DuckDuckGo query expansion, plus optional Ashby/Lever/Greenhouse ATS boards. Each source fails open: if one adapter errors, the run continues with the rest.
...
- `src/engine/preferences.py` extracts a compact preference profile from the stored CV; `src/engine/ranking.py` then uses preferred roles, seniority, must-have signals, location fit, avoid signals, and source quality to rank eligible jobs before Gemini. If profile extraction or diversity selection fails, the pipeline falls back to the stable deterministic global ranking and logs the fallback without exposing private profile text.
- `src/engine/prefilter.py` — cheap deterministic filtering before spending Gemini calls.
- `src/engine/evaluation.py` — model-based scoring and rationale, through the AI provider port.
- `src/engine/ai/` — the AI provider port: `port.py` (the vocabulary core modules use, including the call class that decides which credential and quota fund a call), `credentials.py`, `usage.py` (quota ledger and 429 circuit breaker), `limits.py` (published free-tier limits per model), and `gemini.py` — the only module that knows Gemini exists.
- `src/engine/postgres_store.py` — Postgres persistence (`PostgresJobStore`: dedup, evaluation cache, delivery tracking) against the shared Supabase project.
- `src/engine/cli.py` — one entrypoint per ingestion stage (`crawl-source`, `extract-facets`, `recheck-freshness`, `recover-posting`). There is no `run` entrypoint any more (#189): nothing drives crawl-to-delivery as one process.
...
3. Download the latest `job-hunter-state` artifact (from the most recent successful workflow run,
   before it expires) and run `engine/scripts/migrate_sqlite_to_postgres.py` against it.
...
**Nothing here is required.** A run needs a Gemini API key and nothing else: the published free-tier limits for each supported model are defaults in code (`src/engine/ai/limits.py`), and a model with no entry there runs under the most conservative known limits and logs that it did so.
...
4. That is the whole required setup. The free-tier limits for `GEMINI_MODEL` (defaulting to `gemini-3.5-flash-lite` if unset) come from the table in `src/engine/ai/limits.py`, and a model missing from it runs under the most conservative known limits with a warning in the log. The 429 circuit breaker below is the real safety net either way.
...
uv run python -m engine crawl-source --limit 5
```
