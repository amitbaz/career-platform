...
def _quota(**overrides) -> AIQuotaSettings:
    defaults = dict(rpm=10, tpm=1000, rpd=100)
    defaults.update(overrides)
    return AIQuotaSettings(**defaults)


@pytest.fixture
def tracker(store) -> AIUsageTracker:
    return AIUsageTracker(store, _quota(), MODEL, provider=PROVIDER)
...
# ---------------------------------------------------------------------------
# Step 0: AIQuotaSettings validation
# ---------------------------------------------------------------------------


@pytest.mark.parametrize("field_name", ["rpm", "tpm", "rpd", "rate_pause_seconds"])
@pytest.mark.parametrize("bad_value", [0, -1])
def test_ai_quota_settings_rejects_non_positive_fields(field_name, bad_value):
    kwargs = dict(rpm=10, tpm=1000, rpd=100, rate_pause_seconds=90)
    kwargs[field_name] = bad_value
    with pytest.raises(ValueError, match=field_name):
        AIQuotaSettings(**kwargs)


def test_ai_quota_settings_accepts_all_positive_fields():
    # Regression guard: a valid construction must not raise.
    AIQuotaSettings(rpm=10, tpm=1000, rpd=100, rate_pause_seconds=1)
...
def test_pause_blocks_before_any_budget_check_and_records_no_ledger_row(store, tracker):
    ...
    rows_before = store.ai_usage_rows(
        "2026-09-01T00:00:00+00:00",
        "2026-09-02T00:00:00+00:00",
        provider=PROVIDER,
        model=MODEL,
    ...
    rows_after = store.ai_usage_rows(
        "2026-09-01T00:00:00+00:00",
        "2026-09-02T00:00:00+00:00",
        provider=PROVIDER,
        model=MODEL,
...
def test_record_429_returns_the_persisted_pause_pair(store, tracker):
    """A caller must be able to raise AIQuotaPaused directly from this
    ...
    pause = store.get_ai_pause(PROVIDER, MODEL)
...
def test_record_429_return_value_survives_a_later_expired_or_overwritten_pause(
    ...
    (now rejected by AIQuotaSettings.__post_init__) that made the freshly
    persisted pause already look expired, and the exception surfaced as a
    bare GeminiError instead of AIQuotaPaused. `rate_pause_seconds=0` is
    ...
    store.set_ai_pause(PROVIDER, MODEL, (NOW - timedelta(seconds=1)).isoformat(), "rate_limit")
...
def test_record_429_writes_exactly_one_row_regardless_of_daily_ceiling(store):
    ...
    rpd ceiling, raised AIBudgetExceeded plus a spurious extra
    ...
    tracker = AIUsageTracker(store, tight_quota, MODEL, provider=PROVIDER)
...
def test_snapshot_counts_every_row_the_user_wrote_today(store, tracker):
    """No quota decision reads `run_id` (issue #73).

    The ledger is per user, not per run: a call made by an earlier process
    today spent the same daily quota as one made by this one, so hiding it
    behind a run discriminator would let a run believe its budget was intact.
    """
    store.record_ai_usage(
        occurred_at=NOW.isoformat(),
        provider=PROVIDER,
