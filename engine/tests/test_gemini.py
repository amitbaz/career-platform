...
from engine.ai.usage import AIQuotaPaused, AIUsageTracker
...
def test_generate_text_calls_tracker_preflight_with_purpose_and_prompt(monkeypatch):
    ...
    monkeypatch.setattr("engine.ai.gemini._now", lambda: fixed_now)
...
def test_generate_text_daily_quota_429_pauses_until_pacific_reset(store, monkeypatch):
    ...
    monkeypatch.setattr("engine.ai.gemini._now", lambda: now)
