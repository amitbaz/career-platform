...
from engine.canonical import CanonicalResolver
from engine.content_confidence import AGGREGATOR_TEXT, OFFICIAL_ATS, PARTIAL_UNKNOWN
...
from engine.search_backend import SearchResponse
...
from engine.postgres_store import DryRunStore, PostingBatch
...
def test_collect_candidates_counts_a_row_canonical_resolution_inserts(
    ...
    monkeypatch.setattr(
        "engine.discovery.fetch_authoritative_description",
...
def test_collect_candidates_resolver_tail_costs_only_what_it_resolves(
    ...
    test and `engine/AGENTS.md`, which documents this path as
    per-job.
    """
    from engine.postgres_store import PostgresJobStore
    ...
    assert marginal_per_job >= 1, (
        ...
        "this test and engine/AGENTS.md, which documents this path "
        "as per-job."
    )
...
def test_collect_candidates_measures_every_source_type_comparably(store, policy):
    ...
    """
    from engine.sources.company_watch import CompanyWatchSource
    from engine.sources.learned_ats import LearnedAtsSource
    from engine.sources.lever import LeverSource
    from engine.sources.remotive import RemotiveSource
    from engine.sources.targeted_search import TargetedSearchSource
