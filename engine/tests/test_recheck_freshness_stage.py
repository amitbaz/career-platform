...
from engine.models import Evaluation, Job
from engine.recheck_freshness_stage import RecheckFreshnessStage
...
def test_the_stage_module_imports_nothing_user_scoped():
    ...
    probe = (
        "import sys, engine.recheck_freshness_stage; "
        "loaded = [m for m in ('engine.postgres_store', 'engine.supabase_client', "
        "'engine.config', 'engine.matching', 'engine.evaluation') "
        "if m in sys.modules]; "
        "print(','.join(loaded))"
    )
