...
It is mid-restructure (#181, #189, #287). Ingestion and enrichment run as queued stages on Render
(`render.yaml`); matching is one operation (`match_jobs`, #187); and the older single-process
daily run on GitHub Actions — crawl, score and Telegram digest in one pass — is retired.
Telegram, Gmail intake and on-demand cover-letter/PDF generation are deleted outright, not part
of the product, and there is no backward compatibility to keep with any of them (root
`AGENTS.md`, "Product direction"). Sections below that describe the daily run describe what has
been removed. **The engine never submits applications** — see "v1 safety boundary" in README.md.
...
1. **Postgres is the persistence layer.** The shared Supabase project lives at the repository
   ...
   `scripts/check_job_hunter_shared_tables_doc.py` (run via `pnpm engine:test`),
   ...
   *every* affected user's job row is what the ticket is for and an invoker is scoped by
   ...
   `PostgresJobStore` (`src/engine/postgres_store.py`), reaching PostgREST with a
   ...
   **The one exception is ingestion (#182).** A crawl also holds a direct, pooled Postgres
   connection as a privileged role (`src/engine/pg.py`, configured by `SUPABASE_DB_URL`),
...
**Current production source of truth:** Postgres, via `PostgresJobStore` (`src/engine/postgres_store.py`) against the shared Supabase project.
...
uv run pytest -q                          # run full test suite, serially
uv run pytest -q -n auto --dist=loadgroup # what `pnpm engine:test` and CI actually run (#236)
uv run pytest tests/test_config.py -q     # single file
uv run pytest tests/test_config.py::test_name -q  # single test
...
uv run python -m engine crawl-source --limit 200
uv run python -m engine extract-facets --limit 200
uv run python -m engine recheck-freshness --limit 2000
uv run python -m engine recover-posting --limit 500
```

Local dry run of one stage: copy `.env.example` to `.env`, then `set -a; source .env; set +a` before running. `.env.example` is grouped by the surface each variable serves; a stage needs the Supabase group (`JOB_HUNTER_USER_ID`, `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SIGNING_KEY_B64`, `SUPABASE_DB_URL`). The three free-tier limits (`GEMINI_FREE_RPM`, `GEMINI_FREE_TPM`, `GEMINI_FREE_RPD`) are optional overrides: the published limits per model are defaults in code (`src/engine/ai/limits.py`). Everything under "Optional overrides" can stay blank — each takes the code default stated in its comment, so never copy a default into a value there.

CI (`.github/workflows/engine-ci.yml`) runs `pytest -q -n auto --dist=loadgroup` on Python 3.12 — no lint step configured. It triggers on every pull request, and on pushes to `main` only. The push trigger is deliberately scoped to `main`: without it, a commit on a pull request branch starts both workflows against the same commit and costs twice the Actions minutes, which matters while the repository is private and subject to the monthly cap.
...
- `src/engine/facets.py` — objective extraction (#125): reads a posting's **facets** — the
  ...
  on its own Render cron schedule (`python -m engine extract-facets --limit N`, #189) --
...
- `src/engine/hard_blockers.py` — decides the two objective hard blockers from the facets
...
- `src/engine/evaluation.py` — subjective scoring, the per-user half (#126). Takes a
...
- `DiscoveryStats` also carries the cost half of each source's scorecard: `elapsed_by_source`
  ...
  store-backed sources (company watch, learned ATS) it counts Postgres traffic as
...
- `src/engine/stage_queue.py`, `postgres_stage_queue.py`, `resolve_persist.py` — the
...
- `src/engine/recheck_freshness_stage.py` — the `recheck_freshness` stage (#186). Run
  by `python -m engine recheck-freshness` from its own workflow
  ...
  its employer's own board lists it again.
- `src/engine/worker_runs.py` — worker-run telemetry (#258). Each of the three Render
...
or restores a `job-hunter-state` artifact any more. `concurrency: group: job-hunter-state` no
longer exists anywhere: #189 deleted the daily workflow that carried it, and #287 deletes
`job-hunter-generate-cover-letter.yml`, the one workflow that still carried a copy -- with it goes
the single-writer guarantee that the store's read-then-update pairs were relying on. Ingestion now
runs as Render crons that can overlap (see README, "Ingestion stage schedules").
...
These four secrets must exist in GitHub repository settings — as of this writing they do not
exist yet, so the workflows are non-functional until an operator creates them (see the runbook
in README.md).
